...
def _study(label: str, product: str, sample_every: int):
    df = lob.build_panel(lob.capture_path(label), product=product,
                         sample_every=sample_every, horizon=1, depth=5)
    return df, models.evaluate(df, folds=5, fee_bps=1.0)
...
def study_one(label: str, product: str) -> None:
    ...
    print(f"  {'model':<7} {'IC(spear)':>10} {'gross bps':>10} {'net bps':>9} {'cost bps':>9} {'turnover':>9}")
...
def validate(real_label: str = "2026-07-06", real_product: str = "BTC-USD") -> None:
    ...
    print(f"  {'session':<24} {'best IC':>8} {'gross bps':>10} {'net bps':>10}  verdict")
    for name, label, product, sample_every in sessions:
        ...
        print(f"  {name:<24} {b['ic_spearman']:>+8.3f} {b['gross_ret_bps']:>+10.0f} "
              f"{b['net_ret_bps']:>+10.0f}  {v}")
    print("\nReads as: the models find NOTHING in noise, RECOVER the planted signal (high IC), and a")
    print("real predictive signal in real data — but at tick frequency none survive costs, because")
    print("edge-per-trade < cost-per-trade. IC is not tradability. What DOES survive is low turnover")
    print("(see the cross-sectional momentum result in run_crosssec.py).")
